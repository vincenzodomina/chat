...
describe("parseMessage", () => {
  it("parses a plain text message", () => {
    ...
    expect(message.author.id).toBe("@alice:matrix.org");
...
The `chat` package exports test utilities that make it straightforward to test adapter behavior without real platform APIs.
...
import {
  ...
  createTestMessage,
  mockLogger,
} from "chat/mock-adapter";
```

| Utility | Purpose |
|---------|---------|
| `createMockAdapter(name)` | Creates an `Adapter` with `vi.fn()` mocks for all methods |
| `createMockState()` | Creates a `StateAdapter` with working in-memory subscriptions, locks, and cache |
| `createTestMessage(id, text, overrides?)` | Creates a `Message` object with sensible defaults |
| `mockLogger` | A `Logger` that captures all log calls via `vi.fn()` |

Use `createMockState()` when you need a real state adapter without Redis. Use `createMockAdapter()` when testing the `Chat` class behavior and you need a fake adapter.
